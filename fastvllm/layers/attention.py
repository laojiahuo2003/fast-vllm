...
class Attention(nn.Module):
    ...
    def forward(self, q: torch.Tensor, k: torch.Tensor, v: torch.Tensor):
        ...
        if k_cache.numel() and v_cache.numel(): # numel() 是元素个数,如果cache不为空，就进入
            store_kvcache(k, v, k_cache, v_cache, context.slot_mapping)# 将计算得到的kv，按照slot_mapping放到KV Cache对应的位置
